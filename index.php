...
add_filter( 'acf/format_value/type=relationship', function ( $value ) {
  if(count($value)==0){
    return false;
  }
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'acf-gridsome/v1', '/gridsome', array(
    'methods' => 'GET',
    'callback' => 'get_gridsome_query',
  ) );
} );

function get_gridsome_query(){
  $c="acf {";
  foreach ( acf_get_field_groups() as $group ) {
    $fields = get_posts(array(
        'posts_per_page'   => -1,
      ...
      foreach ( $fields as $field ) {
        ...
        $c.="\n".$field->post_excerpt." { show value label key type }\n";
        //$group["fields"][]=$field->post_excerpt;
      }
